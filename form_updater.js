...
function findFormKey(aquaData, key) {
  ...
  return keys.find(k => k === key || k === `forms-${key}` || k.startsWith(`forms-${key}`));
}
...
function updateForm(filename, key, content) {
  ...
  if (content === undefined) {
    ...
    latestRevision[deletedKey] = latestRevision[formKey];
    delete latestRevision[formKey];
    latestRevision[deletedKey] = ""; // Set value to empty string
    console.log(`Marked ${formKey} as deleted`);
  } else {
    // Update operation
