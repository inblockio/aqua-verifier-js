...
import Aquafier, { AquaTree, LogType, LogTypeEmojis } from "aquafier-js-sdk";
import * as formatter from "./formatter"
import { maybeUpdateFileIndex } from "aqua-protocol";
...
export const estimateWitnessGas = async (wallet_address: string, witness_event_verification_hash: string, ethNetwork: string, smart_contract_address: string, providerUrl: any) => {
    try {
...
export const createRevisionWithMultipleAquaChain = async (timestamp: any, revisionType: string, filename: string) => {
    ...
    if (revisionType == "witness") {
        ...
        const linkAquaFiles = linkURIsArray.map((e: any) => `${e}.aqua.json`)
...
export const revisionWithMultipleAquaChain = async (revisionType: string, filename: string, aquafier: Aquafier, linkURIs: any, enableVerbose: any, enableScalar: any, witness_platform_type: string | undefined, network: string | undefined, witnessMethod: any) => {
...
export function printLogs(logs: any[], enableVerbose: any) {
    if (enableVerbose) {
        logs.forEach((element: { logType: string | number; log: any; }) => {
            console.log(`${LogTypeEmojis[element.logType]} ${element.log}`)
        });
    } else {
        let containsError = logs.filter((element: { logType: string; }) => element.logType == "error");
        if (containsError.length > 0) {
            logs.forEach((element: { logType: string; log: any; }) => {
...
export function checkArrayIntersection(array1: Iterable<unknown> | null | undefined, array2: any[]) {
    // Convert first array to Set for O(1) lookup
    const set = new Set(array1);

    // Check if any element from array2 exists in the set
    const commonElements = array2.filter((item: unknown) => set.has(item));
