...
import { LogType, printLogs } from "aquafier-js-sdk";
...
export const createGenesisRevision = async (aquaFilename, form_file_name, enableScalar, aquafier) => {
    ...
    if (form_file_name) {
        ...
        if (form_file_name != aquaFilename.replace(/\.aqua\.json$/, "")) {
            formatter.log_red(`⛔ First Revision  : Form file name is not the same as the aqua file name `);
...
export const revisionWithMultipleAquaChain = async (revisionType, filename, aquafier, linkURIs, enableVerbose, enableScalar, witness_platform_type, network, witnessMethod, signMethod) => {
    ...
    if (revisionType == "witness") {
        ...
        let creds = readCredentials();
        const signatureResult = await aquafier.signMultipleAquaTrees(aquaObjectWrapperList, signMethod, creds);
